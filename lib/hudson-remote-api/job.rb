module Hudson
  ...
  class Job < HudsonObject
    ...
    attr_accessor :name, :config, :repository_url,
                  :repository_urls, :repository_browser_location,
                  :description

    attr_reader :color, :last_build, :last_completed_build,
                :last_failed_build, :last_stable_build,
                :last_successful_build, :last_unsuccessful_build,
                :next_build_number

    # List all Hudson jobs
    def self.list()
      json = fetch(@@hudson_json_api_path)
    ...
    def load_config()
      @config = fetch(@xml_api_config_path)
